...
pub struct DocumentColorsEvent(pub AppId);
pub struct DocumentLinksEvent(pub AppId);
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        let doc = if doc_matches { Some(&mut self.tabs[self.active_tab].doc) } else { None };
